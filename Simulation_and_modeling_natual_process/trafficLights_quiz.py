...
Tc = 25  # Time latency to change the traffic lights from red to green once a car is found waiting in the queue
Tp = 15  # Passage time
    ...
    def action(self, queue, state):
        if not state.is_green():
            ...
            if state.waiting_cars() == 1:
                queue.insert(
                    R2G(self.t + Tc))  # On the line 79, insert into the queue a R2G event at the time (self.t + Tc)
...
class R2G(Event):
    ...
    def action(self, queue, state):
        ...
        state.purge_cars()  # On the line 87, change the state and turn the light to green
        state.turn_green()
